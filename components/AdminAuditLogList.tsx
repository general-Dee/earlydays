    ...
    <div className="card p-8 md:p-9 shadow-[0_20px_50px_-30px_rgba(22,33,62,0.3)]">
      ...
      {state === "forbidden" && (
        <div className="mt-4 px-3.5 py-3 rounded-lg bg-sun-soft text-clay text-[0.85rem] font-semibold">
          You&rsquo;re logged in, but this account isn&rsquo;t authorized to view the audit log.
        </div>
      )}

      {state === "error" && (
        <div className="mt-4 px-3.5 py-3 rounded-lg bg-clay-soft text-clay text-[0.85rem] font-semibold">
          Couldn&rsquo;t load the audit log. Please try again.
        </div>
      )}

      {state === "ready" && entries.length === 0 && (
        <p className="text-sm text-slate mt-5">No audit log entries yet.</p>
      )}
      ...
      {state === "ready" && entries.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-slate mt-4">No matching entries.</p>
      )}

      {state === "ready" && paged.length > 0 && (
        <ul className="flex flex-col gap-2.5 mt-4">
          {paged.map((entry) => (
            <li key={entry.id} className="px-3.5 py-3 rounded-lg bg-chalk">
              <div className="flex items-center justify-between gap-3">
                <span className="text-sm font-semibold">{entry.action}</span>
                <span className="text-xs text-slate">{new Date(entry.createdAt).toLocaleString("en-NG")}</span>
              </div>
              <p className="text-sm mt-1 mb-0 text-slate">
                {entry.actorEmail}
                {(entry.targetEmail || entry.targetUid) && <> &rarr; {entry.targetEmail ?? entry.targetUid}</>}
              </p>
              {entry.detail && <p className="text-[0.8rem] mt-1 mb-0 text-slate">{entry.detail}</p>}
            </li>
          ))}
        </ul>
      )}
