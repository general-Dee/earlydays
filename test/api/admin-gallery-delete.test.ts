...
function request(headers: Record<string, string> = {}) {
  return new NextRequest("http://localhost/api/admin/gallery/g1", { method: "DELETE", headers });
}
...
describe("DELETE /api/admin/gallery/[id]", () => {
  it("rejects requests without an Authorization header", async () => {
    ...
    const { DELETE } = await import("@/app/api/admin/gallery/[id]/route");
  ...
  it("deletes the Storage photo and the Firestore doc when the photo exists", async () => {
    get.mockResolvedValue({ exists: true, data: () => ({ photoStoragePath: "gallery/g1/g1-1.jpg" }) });
    ...
    expect(file).toHaveBeenCalledWith("gallery/g1/g1-1.jpg");
    expect(fileDelete).toHaveBeenCalled();
    expect(del).toHaveBeenCalled();
  });

  it("still deletes the Firestore doc when the record has no photoStoragePath", async () => {
